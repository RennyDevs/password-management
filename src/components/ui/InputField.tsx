...
const InputField = forwardRef<HTMLInputElement, InputFieldProps>(
  ({ label, error, hint, rightAction, showPasswordToggle, type, className = '', ...props }, ref) => {
    const autoId = useId();
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <input
            ...
            className={`${error ? 'vault-input-error' : 'vault-input'} pr-${showPasswordToggle || rightAction ? '10' : '4'} ${className}`}
            {...props}
          />
          {(showPasswordToggle || rightAction) && (
